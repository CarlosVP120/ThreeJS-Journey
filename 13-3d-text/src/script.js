...
// // one way to import a font
// import typefaceFont from "three/examples/fonts/helvetiker_regular.typeface.json";

// another way to import a font
...
// to create the geomertry of the text
...
fontLoader.load("/fonts/helvetiker_regular.typeface.json", (font) => {
  const textGeometry = new TextGeometry("Hello Three.js", {
    ...
    size: 0.5,
    height: 0.2,
    // defines the number of points on the curve (more = smoother)
    curveSegments: 5,
    bevelEnabled: true,
    bevelThickness: 0.03,
    bevelSize: 0.02,
  ...
  // OR center the text with center()
  textGeometry.center();

  const material = new THREE.MeshMatcapMaterial({ matcap: matcapTexture });
  const text = new THREE.Mesh(textGeometry, material);

  // Let's add objects floating around.
